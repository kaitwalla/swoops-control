...
func (s *Service) Connect(stream agentrpc.AgentService_ConnectServer) error {
	...
	now := s.now()
	if err := s.store.UpsertHostHeartbeat(hello.HostID, hello.AgentVersion, hello.OS, hello.Arch, hello.HostName, hello.AgentUser, now); err != nil {
